...
class ScopeAction {
  ...
  async run(theScope, theTokens, theLine, theDoc, trace) {
    if (trace) console.log(`running action: ${theScope}`)
    return await this.func(
      this.scope, theScope, theTokens, theLine, theDoc, trace
    )
  }
}
  ...
  static async loadActionsFrom(aDir, config) {
    ...
    for await (const dirEnt of openedDir) {
      ...
      actions2Load.push(async function() {
        ...
        aModule.registerActions(config, Config, Builders, Grammars, ScopeActions, Structures)
