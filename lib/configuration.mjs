...
class Config {
  ...
  static getDescriptionDir() {
    var descPaths = ['.']
    const bConf = Config.theConfig['build']
    if (!bConf) return descPaths
    const buildDir = bConf['buildDir']
    if (!bConf['descPaths']) return descPaths
    if (!bConf['descPaths'] instanceof Array) return descPaths
    descPaths = bConf['descPaths']
    if (buildDir) {
      const newDescPaths = []
      for (const aDescPath of descPaths) {
        newDescPaths.push(aDescPath.replaceAll('$buildDir', buildDir))
      }
      descPaths = newDescPaths
    }
    const newPaths = []
    for (const aPath of descPaths) {
      newPaths.push(this.normalizePath(aPath))
    }
    return newPaths
  }

  static getFirstDescriptionDir() {
    var aDescPath = '.'
    const thePaths = Config.getDescriptionDir()
    if (!thePaths) return aDescPath
    if (thePaths.length < 1) return aDescPath
    return thePaths[0]
  }

  static async loadConfigFromFile(configPath, verbose) {
    var configText = ""
    if (verbose) {
    ...
    var fileConfig = {}
    if (configText) {
      if (verbose) console.log(`Loaded configuration from [${configPath}]`)
  ...
  static async loadConfig(cliArgs, configBaseName, defaultConfig, funcObj) {
    ...
    if (!cliOpts.config) {
      for (const anExt of ['.yaml', '.yml', '.toml', '.json']) {
        ...
        fConfig = await Config.loadConfigFromFile(cliOpts.config, verbose)
    ...
    if (config['cfdoitConfig']) {
      var cfdoitConfig = await Config.loadConfigFromFile(
        Config.normalizePath('~/.config/cfdoit/config.toml'), verbose
      )
      ...
      cfdoitConfig = await Config.loadConfigFromFile(
        Config.normalizePath(config['cfdoitConfig']), verbose
      )
      if (0 < Object.keys(cfdoitConfig).length) {
        config = deepmerge(config, cfdoitConfig)
      }
    ...
    try {
      ...
      console.log("Failed to run preLoadFunc!")
      console.log(err)
    }
